...
6. numbers: 제품을 식별하는 숫자만 (예: ["23", "39", "45"])
    - 중요: 한자 숫자는 아라비아 숫자로 변환 (예: "二割三分" → "23", "三割九分" → "39", "四割五分" → "45")
    - 포함할 것: 정미비율, 등급, 숙성 연수처럼 제품명을 구분하는 숫자
    - 제외할 것: 알코올 도수, 용량, 전화번호, 우편번호, 주소 번지, 관리번호, 제조일자
    - 해당하는 숫자가 없으면 빈 배열로 두세요. 억지로 채우지 마세요.
7. volume: 용량 (예: "720ml", "1800ml", "1.8L", "900ml")
8. rawText: 제품 식별에 쓰이는 핵심 텍스트만 100자 이내
    - 포함할 것: 브랜드명, 양조장명, 등급 표기(純米大吟醸, 本格焼酎 등)
    - 제외할 것: 원재료명, 주소, 전화번호, URL, 관리번호, 음주 경고문, 보관·침전물 안내
...
8. numbers: 제품을 식별하는 숫자만 (예: ["2018"] — 빈티지 연도 등)
    - 제외할 것: 알코올 도수, 용량, 전화번호, 우편번호, 주소 번지, 관리번호
    - 해당하는 숫자가 없으면 빈 배열로 두세요.
9. volume: 용량 (예: "750ml", "1.5L", "375ml")
10. rawText: 제품 식별에 쓰이는 핵심 텍스트만 100자 이내
    - 포함할 것: 와이너리명, 브랜드명, 등급 표기(Grand Cru, Reserva 등), 지역·품종 표기
    - 제외할 것: 주소, 전화번호, URL, 수입사 정보, 음주 경고문, 보관 안내
