...
| 9-d | 업로드 임베딩을 큐 경로로 (VPN 불필요) | 지역 제한 우회 | $0 | 9.2 | **미착수 — 다음 라운드 권장 1순위** |
...
### 9.2 [추가] Gemini API 지역 제한 — 실행 경로에 따라 갈린다

임베딩 호출이 **실행 경로에 따라** `User location is not supported for the API use`(400 FAILED_PRECONDITION)로 거절됩니다. Cloudflare Workers는 요청이 도달한 엣지 위치에서 외부 호출을 내보내는데, 관리자 HTTP 요청이 도달하는 위치는 Gemini가 허용하지 않고 큐 컨슈머가 실행되는 위치는 허용합니다. 같은 API 키, 같은 코드입니다.

- 사진 검색(큐 경로)은 항상 정상 동작합니다.
- 평가는 큐 경로로 옮겨 해결했습니다.
- **엑셀 업로드(`admin.ts`)는 아직 HTTP 핸들러 경로에서 `getBatchEmbeddings`를 호출하므로 VPN 없이는 실패합니다.** 지금까지 업로드마다 VPN으로 우회해 온 원인이 이것입니다. 업로드 청크 처리를 큐로 옮기면 해소됩니다(9-d).
- 참고: `wrangler.toml`의 Smart Placement는 백엔드 근접 배치 휴리스틱이지 국가 지정 기능이 아니므로 이 문제의 해법이 되지 못합니다.
...
1. **업로드를 큐 경로로** (9-d) — VPN 없이 데이터 갱신이 가능해지고, 그래야 와인 데이터를 넣어 평가가 5/5가 됩니다.
